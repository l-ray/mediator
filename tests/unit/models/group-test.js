...
describeModel(
  ...
  function() {
      ...
      it('should not interfere on pictures of other groups)', function(){
        var store = this.store();
        var item1 = store.createRecord('group',{});
        var item2 = store.createRecord('group',{});

        var result_1_1 = store.createRecord('result', {});
        result_1_1.get('pictures')
          .pushObject(store.createRecord('picture', {'url':'http://test.co/img1.src'}));

        var result_1_2 = store.createRecord('result', {});
        var p2 = result_1_2.get('pictures');
        p2.pushObject(store.createRecord('picture', {'url':'http://test.co/img2.src'}));
        p2.pushObject(store.createRecord('picture', {'url':'http://test.co/img3.src'}));

        var result_2_1 = store.createRecord('result', {});
        var p3 = result_2_1.get('pictures');
        p3.pushObject(store.createRecord('picture', {'url':'http://test.co/img4.src'}));
        p3.pushObject(store.createRecord('picture', {'url':'http://test.co/img5.src'}));

        var results = item1.get('results');

        results.pushObject(result_1_1);
        results.pushObject(result_1_2);
        item1.enumerableContentDidChange();

        item2.get('results').pushObject(result_2_1);
        item2.enumerableContentDidChange();

        expect(item1.get('pictures')).to.have.an.property('length');
        expect(item1.get('pictures')).to.be.not.empty;
        assert.lengthOf(item1.get('pictures'),3);

        expect(item2.get('pictures')).to.have.an.property('length');
        expect(item2.get('pictures')).to.be.not.empty;
        assert.lengthOf(item2.get('pictures'),2);
